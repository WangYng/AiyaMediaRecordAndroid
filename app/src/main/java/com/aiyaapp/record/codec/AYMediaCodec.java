package com.aiyaapp.record.codec;
...
public class AYMediaCodec {
    ...
    public AYMediaCodec(String path, int trackCount) {
        // 创建音视频合成器
     ...
     * 配置和启用视频编码器
     */
    public boolean configureVideoCodecAndStart(final int width, final int height, int bitrate, int fps, int iFrameInterval) {
        if (width % 16 != 0 && height % 16 != 0) {
            Log.w(AYGPUImageConstants.TAG, "videoWidth = " + width + " height = " + height + " Compatibility is not good");
        }
    ...
    public boolean configureAudioCodecAndStart(int bitrate, int sampleRate, int channelCount) {
        MediaFormat format = MediaFormat.createAudioFormat(AYMediaCodecHelper.MIME_TYPE_AUDIO, sampleRate, channelCount);
    ...
    private void initEGLContext() {
        eglContext.syncRunOnRenderThread(new Runnable() {
            @Override
            public void run() {
                eglContext.bindEGLWindow(inputSurface);
    ...
    public void writeImageTexture(final int texture, final int width, final int height, final long timeStamp) {
        ...
        final long time = timeStamp - videoStartTime - videoFrameDelayTime * 1000 * 1000;
        ...
        eglContext.syncRunOnRenderThread(new Runnable() {
            ...
            public void run() {
                ...
                eglContext.setTimeStemp(time < 0 ? 0 : time);
    ...
    public void writePCMByteBuffer(ByteBuffer source, final long timeStamp) {
        ...
        long time = timeStamp - audioStartTime;
        ...
        if (inputSurface != null) {
            ...
            eglContext.syncRunOnRenderThread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (eglContext != null) {
                        eglContext.destroyEGLWindow(inputSurface);
                    }
                }
            });
        }
    ...
    private static class AYMp4Muxer {
        ...
        void addData(int trackIndex, ByteBuffer buffer, MediaCodec.BufferInfo info) {
            ...
            if (trackCount == maxTrackCount) {
                ...
                Log.d(AYGPUImageConstants.TAG, "writeSampleData " + info.size);
    ...
    public void setVideoFrameDelayTime(long videoFrameDelayTime) {
        this.videoFrameDelayTime = videoFrameDelayTime;
    }
}
